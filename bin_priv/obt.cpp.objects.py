#!/usr/bin/env python3
"""
Search for C++ classes and structs (objects) in database.
...
class ObjectsCommand(CppCommandBase):
    """Command for searching C++ classes and structs"""
    ...
    def create_parser(self):
        ...
        parser.add_argument(
            'pattern',
            nargs='?',
            help='Pattern to search for in class/struct names'
        )
        ...
        parser.add_argument(
            '--templates-only',
            action='store_true',
            help='Show only template classes/structs'
        )
    ...
    def run(self, args):
        ...
        try:
            # Force types to be class,struct
            args.types = 'class,struct'
            ...
            if args.json:
                ...
                print(f"\n{deco.green(f'Found {len(results)} classes/structs')}")
